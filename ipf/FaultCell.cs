...
namespace ipf
{
    public class FaultCell
    {
        ...
        */
        ...
        float size, ColorMap cmap, FaultCell[] cells, bool lhc)
        {
            return getXyzUvwRgb(size, cmap, cells, (FaultCell cell) => cell.s1, lhc);
        }
        ...
        */
        ...
        float size, ColorMap cmap, FaultCell[] cells, bool lhc)
        {
            return getXyzUvwRgb(size, cmap, cells, (FaultCell cell) => cell.fl, lhc);
        }

        /////////////////////////////////////////////////////////////////////////
        // package

        internal int i1, i2, i3; // cell indices
        internal float x1, x2, x3; // cell coordinates
        internal float fl, fp, ft; // likelihood, strike (phi) and dip (theta)
        internal float u1, u2, u3, us; // dip vector and scale factor = 1/Math.Sin(theta)
        internal float v1, v2, v3; // strike vector
        internal float w1, w2, w3; // normal vector
        internal FaultCell ca, cb, cl, cr; // nabors above, below, left and right
        internal FaultSkin skin; // if not null, the skin to which this cell belongs
        internal int i2m, i2p; // sample indices i2 for minus and plus sides of cell
        internal int i3m, i3p; // sample indices i3 for minus and plus sides of cell
        internal float[] emp; // array of minus-plus alignment errors
        internal float smp; // shift from minus side to plus side of cell
        internal float s1, s2, s3; // fault dip-slip vector

        internal delegate float Get1(FaultCell cell);
        internal delegate float[] GetN(FaultCell cell);
        internal delegate void Set1(FaultCell cell, float value);
        internal delegate void SetN(FaultCell cell, float[] values);
        ...
        */
        ...
        {
            return (float)Math.Sqrt(distanceSquaredTo(p1, p2, p3));
        }
        ...
        * @return distance squared.
        */
        internal float distanceSquaredTo(float p1, float p2, float p3)
        {
            float d1 = p1 - x1;
            float d2 = p2 - x2;
            float d3 = p3 - x3;
