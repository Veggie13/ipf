...
namespace ipf
{
    public class FaultScanner
    {
        ...
        */
        ...
        return vf.Select(f => vfscl * f).ToArray();
        }
        ...
        */
        ...
        int n2 = f[0].Length;
        int n3 = f.Length;
        float p2min = (float)(-slopeMax);
        float p2max = (float)( slopeMax);
        float p3min = (float)(-slopeMax);
        float p3max = (float)( slopeMax);
        ...
        */
        ...
        t1[i1] = (float)(0.54+0.46*Math.Cos(Math.PI*(m1-i1)/m1));
        for (int i2=0; i2<m2; ++i2)
        t2[i2] = (float)(0.54+0.46*Math.Cos(Math.PI*(m2-i2)/m2));
        for (int i3=0; i3<m3; ++i3)
        t3[i3] = (float)(0.54+0.46*Math.Cos(Math.PI*(m3-i3)/m3));
        for (int i3=0; i3<n3; ++i3) {
        for (int i2=0; i2<n2; ++i2) {
        ...
        */
        ...
        if (fti!=NO_DIP)
        gt[i3][i2][i1] = ArrayMath.toDegrees((float)Math.Atan(scale*(float)Math.Tan(ArrayMath.toRadians(fti))));
        }
        }
        }
        return gt;
        ...
        */
        ...
        private const float NO_STRIKE = -0.00001f;
        private const float NO_DIP    = -0.00001f;
        ...
        int n1 = snd[0][0][0].Length;
        int n2 = snd[0][0].Length;
        int n3 = snd[0].Length;
        ...
        {
        double fa = amin;
        double da = ArrayMath.toDegrees(0.5/sigma);
        ...
        float cmax = Math.Cos(toRadians(amin));
        float wwmax = cmax*cmax; 
        for (int i3=0,j3=n3-1; i3<imax; ++i3,--j3) {
        for (int i2=0; i2<n2; ++i2) {
        for (int i1=0; i1<n1; ++i1) {
        float fi = f[i3][i2][i1];
        float Math.PI = p[i3][i2][i1];
        float ti = t[i3][i2][i1];
        float fj = f[j3][i2][i1];
        float pj = p[j3][i2][i1];
        float tj = t[j3][i2][i1];
        if (fi!=0.0f) {
        float[] w = faultNormalVectorFromStrikeAndDip(Math.PI,ti);
        float w3 = w[2];
        if (w3*w3>wwmax) {
        f[i3][i2][i1] = 0.0f;
        p[i3][i2][i1] = 0.0f;
        ...
        */
        private bool inBounds(double x2p, double x3p) {
